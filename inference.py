...
def predict(stats_dict):
    """
    Takes a stats dict (same format as the 'statistics' column in your dataset)
    and returns a full prediction with confidence scores.
    """

    # Flatten the stats dict into a single row — same as DataPrep
    row = {}
    ...
    # ── Stage 1: is there an anomaly? ─────────────────────────────────────────
    anomaly_prob  = model1.predict_proba(X)[0][1]  # probability of being anomaly
    ...
    return {
        ...
        "type_distribution": {k: round(float(v), 4) for k, v in
                              sorted(type_probs.items(), key=lambda x: x[1], reverse=True)}
    }

# ── Test it on real samples from your dataset ──────────────────────────────────
def run_pipeline_test():
    df = pd.read_csv("data.csv")

    # Parse the statistics column
    df["stats_parsed"] = df["statistics"].apply(
        lambda s: eval(s, {"array": np.array, "np": np})
    )
    df["anomalies_parsed"] = df["anomalies"].apply(
        lambda s: eval(s, {"array": np.array, "np": np})
    )
    ...
    # Grab 5 normal and 5 anomalous samples to test
