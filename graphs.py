...
# ============= COLLECT STATISTICS FROM ENTIRE DATASET =============
anomaly_types = []
anomaly_exists = []
zones = []
applications = []

# Collect all KPI stats across all rows
...
# ============= GRAPH 1: ANOMALY TYPES PIE CHART =============
...
wedges, texts, autotexts = ax.pie(sizes, colors=colors_anomaly, autopct='%1.1f%%',
...
info_text = "Anomaly Type Counts\n" + "━" * 35 + "\n\n" + "\n".join([f"{atype:30s}: {count:6d}" for atype, count in
                                                                     sorted(anomaly_type_counts.items(),
                                                                            key=lambda x: x[1], reverse=True)])
ax2.text(0.05, 0.95, info_text, transform=ax2.transAxes, fontsize=12,
         verticalalignment='top', fontfamily='monospace', fontweight='bold',
         bbox=dict(boxstyle='round', facecolor='lightblue', alpha=0.9, pad=2, linewidth=2))

fig.suptitle(f'Distribution of Anomaly Types (Entire Dataset - {len(df)} windows)',
             fontsize=14, fontweight='bold', y=0.98)

plt.tight_layout()
plt.savefig('D:/PFE/graph1_anomaly_types.png', dpi=150, bbox_inches='tight')
print("✅ Saved: graph1_anomaly_types.png")
plt.close()
...
wedges, texts, autotexts = ax.pie(sizes_tf, labels=labels_tf, colors=colors_tf, autopct='%1.1f%%',
...
ax.set_title(f'Anomaly Presence Distribution (Entire Dataset)',
             fontsize=14, fontweight='bold', pad=20)

# Add count
info_text = f"Anomaly: {anomaly_counts[True]}\nNormal: {anomaly_counts[False]}"
ax.text(1.2, 0.5, info_text, transform=ax.transAxes, fontsize=11,
        verticalalignment='center', fontfamily='monospace',
        bbox=dict(boxstyle='round', facecolor='lightyellow', alpha=0.8))

plt.tight_layout()
plt.savefig('D:/PFE/graph2_anomaly_tf.png', dpi=150, bbox_inches='tight')
print("✅ Saved: graph2_anomaly_tf.png")
plt.close()
...
wedges, texts, autotexts = ax.pie(zone_sizes, labels=zone_labels, colors=colors_zones, autopct='%1.1f%%',
                                  startangle=90, textprops={'fontsize': 12, 'fontweight': 'bold'})
...
ax.set_title(f'Network Zones Distribution (Entire Dataset)',
             fontsize=14, fontweight='bold', pad=20)
...
plt.tight_layout()
plt.savefig('D:/PFE/graph3_zones.png', dpi=150, bbox_inches='tight')
print("✅ Saved: graph3_zones.png")
plt.close()
...
wedges, texts, autotexts = ax.pie(app_sizes, labels=app_labels, colors=colors_apps, autopct='%1.1f%%',
                                  startangle=90, textprops={'fontsize': 11, 'fontweight': 'bold'})
...
ax.set_title(f'Application Types Distribution (Entire Dataset)',
             fontsize=14, fontweight='bold', pad=20)

# Add count
info_text = "\n".join([f"{app}: {c}" for app, c in app_counts.items()])
ax.text(1.2, 0.5, info_text, transform=ax.transAxes, fontsize=10,
        verticalalignment='center', fontfamily='monospace',
        bbox=dict(boxstyle='round', facecolor='lightyellow', alpha=0.8))

plt.tight_layout()
plt.savefig('D:/PFE/graph4_applications.png', dpi=150, bbox_inches='tight')
print("✅ Saved: graph4_applications.png")
plt.close()
...
# Category 3: Modulation & Resources
...
traffic_kpis = ['TX_Bytes', 'RX_Bytes', 'Estimated_UL_Buffer', 'PRB_Utilization_DL', 'PRB_Utilization_UL',
...
plt.tight_layout()
plt.savefig('D:/PFE/graph5_kpi_means_overall.png', dpi=150, bbox_inches='tight')
print("✅ Saved: graph5_kpi_means_overall.png")
plt.close()
...
ax.set_xscale('log')
ax.set_title('KPI Variability - Overall Dataset\n(Log scale: TX_Bytes variance is 8.8M, BLER variance is near 0)',
             fontsize=12, fontweight='bold', pad=15)
ax.grid(True, alpha=0.3, axis='x', which='both')

from matplotlib.patches import Patch

legend_elements = [Patch(facecolor='#d62728', label='High (>100)'),
                   Patch(facecolor='#1f77b4', label='Medium'),
                   Patch(facecolor='#2ca02c', label='Low (<1)')]
ax.legend(handles=legend_elements, loc='lower right', fontsize=10)

plt.tight_layout()
plt.savefig('D:/PFE/graph6_variance_overall.png', dpi=150, bbox_inches='tight')
print("✅ Saved: graph6_variance_overall.png")
plt.close()
...
plt.tight_layout()
plt.savefig('D:/PFE/graph8_anomaly_zone_app.png', dpi=150, bbox_inches='tight')
print("✅ Saved: graph8_anomaly_zone_app.png")
plt.close()
print("\n" + "=" * 70)
